...
select t1.bidder_id, (t2.wins/t1.tol)*100 as per from (
select bidder_id, count(bidder_id) as tol from ipl_bidding_details
group by bidder_id order by bidder_id) as t1 
join
(select bidder_id, count(bid_status) as wins from ipl_bidding_details
where bid_status ='Won' group by bidder_id order by bidder_id) as t2
...
select ms.no_of_matches, std.stadium_name, std.city from IPL_Stadium std join (
select stadium_id, count(match_id) as no_of_matches from ipl_match_schedule
group by stadium_id) as ms on std.stadium_id = ms.stadium_id
order by ms.no_of_matches;

-- Q3. In a given stadium, what is the percentage of wins by a team which has won the toss?

select f1.stadium_id, f2.stadium_name, f1.per from
(select t1.stadium_id, (t1.win/t2.tol)*100 per from 
(select t1.stadium_id, count(t1.stadium_id) win from ipl_match_schedule t1 join
(select match_id from ipl_match where toss_winner = match_winner) t2
on t1.match_id = t2.match_id group by t1.stadium_id order by t1.stadium_id) t1 join 
(select stadium_id, count(stadium_id) tol from ipl_match_schedule group by stadium_id
order by stadium_id) t2 on t1.stadium_id = t2.stadium_id
order by t1.stadium_id) f1 join ipl_stadium f2 on f1.stadium_id = f2.stadium_id;
...
select IT.team_id, IT.team_name, BD.tol_bids from IPL_Team as IT join 
(select bid_team, count(bidder_id) as tol_bids from IPL_Bidding_Details group by bid_team order by bid_team) as BD 
...
-- Option 1:
select team_id, team_name, win_details from IPL_MATCH t1 join ipl_team t2
on t1.win_details like concat('%',t2.remarks,'%');

-- substring from start to end:
select substr(substr(win_details, 6), 1, locate(' ', substr(win_details, 6))-1) as short_form from IPL_MATCH ;

-- Option 2:
select t2.team_id, t2.team_name, t1.win_details from 
(select win_details, substr(substr(win_details, 6), 1, locate(' ', substr(win_details, 6))-1) as short_wd from IPL_MATCH) t1
join ipl_team t2
on t2.remarks = t1.short_wd;
...
select t1.team_id, t2.team_name, t1.mp, t1.mw, t1.ml from
(select team_id, sum(matches_played) mp, sum(matches_won) mw, sum(matches_lost) ml from IPL_Team_Standings
group by team_id) t1 join IPL_Team t2 on t1.team_id = t2.team_id;

-- ----------------------------------------
-- select t1.team_id1, t1.cnt_team_id1 + t2.cnt_team_id2 as tol from 
-- (select team_id1, count(team_id1) cnt_team_id1 from ipl_match
-- group by team_id1
-- order by team_id1) t1 join

-- (select team_id2, count(team_id2) cnt_team_id2 from ipl_match
-- group by team_id2
-- order by team_id2) t2
-- on t1.team_id1 = t2.team_id2;

-- select match_winner, count(match_winner) from ipl_match 
-- group by match_winner;

-- select toss_winner, count(toss_winner) from ipl_match
...
select t1.team_id, t1.team_name, t2.no_of_bowlers from ipl_team t1 join
(select team_id, count(player_role) no_of_bowlers from IPL_Team_players
where player_role = 'Bowler'
group by team_id) t2 on t1.team_id = t2.team_id
where t1.team_name = 'Mumbai Indians';
...
select t1.team_id, t1.team_name, t2.no_of_alrnd from ipl_team t1 join
(select team_id, count(player_role) no_of_alrnd from IPL_Team_players
where player_role = 'all-rounder'
group by team_id) t2 on t1.team_id = t2.team_id
where t2.no_of_alrnd > 4
order by t2.no_of_alrnd desc;
